...
默认情况下，会话（session）处理程序将始终将会话的 cookie 添加到 HTTP 响应中， 即便会话未被您的应用程序所访问。
如需仅在使用会话时创建会话 cookie，请使用
...
您需要添加 `io.vertx：vertx-auth-jwt：4.3.8` 依赖项才能使用 `JWTAuthProvider`。
...
`io.vertx:vertx-web-templ-mvel:4.3.8`。 并通过此方法以创建 MVEL 模板引擎实例：
...
`io.vertx:vertx-web-templ-jade:4.3.8`。 并通过此方法以创建 Jade 模板引擎实例：
...
`io.vertx:vertx-web-templ-handlebars:4.3.8`。 并通过此方法以创建 Handlebars 模板引擎实例：
...
`io.vertx:vertx-web-templ-thymeleaf:4.3.8`。 并通过此方法以创建 Thymeleaf 模板引擎实例：
...
`io.vertx:vertx-web-templ-freemarker:4.3.8`。 并通过此方法以创建 Apache FreeMarker 模板引擎实例：
...
`io.vertx:vertx-web-templ-pebble:4.3.8`。 并通过此方法以创建 Pebble 模板引擎实例：
...
您需要在项目中添加以下 _依赖_ 以使用 Rocker：`io.vertx:vertx-web-templ-rocker:4.3.8
...
`io.vertx:vertx-web-templ-httl:4.3.8`。 并通过此方法以创建 HTTL 模板引擎实例：
...
`io.vertx:vertx-web-templ-rythm:4.3.8`。 并通过此方法以创建 Rythm 模板引擎实例：
