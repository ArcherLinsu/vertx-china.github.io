...
----
compile 'io.vertx:vertx-micrometer-metrics:4.3.5'
...
Vert.x 默认情况下不开启 SPI 的实现。
您必须在 Vert.x options 中启用。
...
[[_connecting_to_influxdb_v2]]
==== 连接到 InfluxDB V2

如果您要连接到 InfluxDB V2 ，那么最少需要设置 `link:../../apidocs/io/vertx/micrometer/VertxInfluxDbOptions.html#setOrg-java.lang.String-[org]` 这个属性。
...
[[_using_a_specific_database_name_or_bucket]]
==== 使用指定的数据库或桶
...
对于 InfluxDB V2 来说，您一样可以使用 `link:../../apidocs/io/vertx/micrometer/VertxInfluxDbOptions.html#setBucket-java.lang.String-[setBucket]` 函数来设置指定的桶。
