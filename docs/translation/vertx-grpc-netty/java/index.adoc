...
NOTE: 您也可以使用其它压缩器，只要服务器支持，在构建 `ManagedChannel` 时在压缩器注册表里注册它们即可
...
IMPORTANT: 由于 gRPC 使用 HTTP/2 传输，SSL/TLS 设置需要在您的服务端中开展 https://wikipedia.org/wiki/Application-Layer_Protocol_Negotiation[应用层协议协商]
...
首先，我们需要有 RPC 定义，这应该已经完成，否则将没有服务器端，并且应该编译相同的定义。
