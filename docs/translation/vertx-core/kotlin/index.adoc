...
不同于其他语言， Java 没有为 http://json.org/[JSON] 提供一等公民的支持，所以我们提供了两个类，使 Vert.x 应用中使用 JSON 更加容易

`link:../../apidocs/io/vertx/core/json/JsonObject.html[JsonObject]` 类用来表示 JSON 对象， `link:../../apidocs/io/vertx/core/json/JsonArray.html[JsonArray]` 类用来表示数组，可以参考 Java 文档获得更多信息。
